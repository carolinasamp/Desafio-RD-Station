...
Além dos requisitos propostos no desafio, foram realizadas as seguintes melhorias:

- Implementação da lógica de recomendação para os modos `SingleProduct` e `MultipleProducts`.
- Organização da regra de negócio em funções menores para facilitar manutenção e extensão.
- Implementação de testes unitários para o serviço de recomendação.
- Validação do formulário, desabilitando o envio enquanto os campos obrigatórios não estiverem preenchidos.
- Melhorias visuais utilizando Tailwind CSS.
- Ajustes de responsividade para melhor experiência em dispositivos móveis.

## Autor

Desenvolvido por Carolina Sampaio Rocha
